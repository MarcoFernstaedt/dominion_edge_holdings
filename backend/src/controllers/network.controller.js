...
export function getRelationshipGraph(req, res) {
  try {
...
export function getHighValueRelationships(req, res) {
  try {
    const contacts   = store.contacts ?? [];
    const edges      = store.relationshipEdges ?? [];
...
export function getNetworkContext(req, res) {
  try {
    const contacts   = store.contacts ?? [];
    const edges      = store.relationshipEdges ?? [];
...
export function calcNextMove(req, res) {
  try {
...
export async function createIntroDraft(req, res) {
  try {
    const contact = store.contacts.find((c) => c.id === req.params.id);
    if (!contact) return errorResponse(res, 404, 'NOT_FOUND', 'Contact not found');
...
export function createRelationshipEdge(req, res) {
  try {
...
export function listRelationshipEdges(req, res) {
  try {
    const edges = store.relationshipEdges ?? [];
...
export function getIntroPaths(req, res) {
  try {
...
export function scorePaths(req, res) {
  try {
...
export function getNetworkAlerts(req, res) {
  try {
    const contacts   = store.contacts ?? [];
    const edges      = store.relationshipEdges ?? [];
    const adjacency  = RelationshipGraph.buildAdjacencyMap(edges);
...
export function getCredibility(req, res) {
  try {
    const boardState = BoardSeatEngine.calcBoardReadinessScore(store.boardSeats ?? [], store.boardCandidates ?? []);
...
export function getCommandCenterNetwork(req, res) {
  try {
    const contacts   = store.contacts ?? [];
    const edges      = store.relationshipEdges ?? [];
    const adjacency  = RelationshipGraph.buildAdjacencyMap(edges);
