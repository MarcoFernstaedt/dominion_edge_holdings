...
export function getApprovalStaleness(req, res) {
  try {
...
export const approveApprovalValidate = validate(z.object({ notes: z.string().max(500).optional() }));
...
export const rejectApprovalValidate = validate(z.object({ reason: z.string().min(1).max(500) }));
...
export const reviseApprovalValidate = validate(z.object({ instructions: z.string().min(1).max(1000) }));
export function reviseApproval(req, res) {
  try {
...
export function applyApproval(req, res) {
  try {
...
export const createArtifactValidate = validate(z.object({
...
}).refine((d) => d.type || d.artifactType, { message: 'type is required', path: ['type'] }));
...
export function getArtifactSummary(req, res) {
  try {
...
export function getArtifactVersions(req, res) {
  try {
    const art = ArtifactStore.getById(req.params.id);
    if (!art) return res.status(404).json({ error: 'Artifact not found' });
...
export function getArtifactStaleness(req, res) {
  try {
...
export function markArtifactSent(req, res) {
  try {
...
export const generateArtifactValidate = validate(z.object({
...
export async function generateArtifact(req, res) {
  try {
...
export async function regenerateArtifact(req, res) {
  try {
...
export const archiveArtifactValidate = validate(z.object({
  by:     z.string().min(1),
  reason: z.string().max(500).optional(),
}));
export function archiveArtifact(req, res) {
  try {
...
export const exportArtifactValidate = validate(z.object({
  export_type:    z.string().min(1),
  requested_by:   z.string().min(1),
  destination:    z.string().optional(),
  export_options: z.record(z.unknown()).optional(),
}));
export function exportArtifact(req, res) {
  try {
