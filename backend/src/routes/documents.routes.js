...
router.post('/api/documents', validate(DocumentSchema), (req, res) => {
  try {
    const doc = {
      id: uid(),
      createdAt: nowIso(),
...
router.get('/api/reports/summary', (_req, res) => {
  try {
    const now = new Date();
...
router.patch('/api/settings', validate(SettingsPatchSchema), (req, res) => {
  try {
...
router.post('/api/ai/reply-suggestion', validate(ReplySuggestionSchema), async (req, res) => {
