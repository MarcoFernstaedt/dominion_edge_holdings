...
router.get('/api/deal-feed', (req, res) => {
  try {
    const {
...
router.get('/api/deal-feed/:id', (req, res) => {
  try {
...
router.post('/api/deal-feed', validate(DealFeedListingSchema), (req, res) => {
  try {
...
router.patch('/api/deal-feed/:id', validate(DealFeedListingPatchSchema), (req, res) => {
  try {
...
router.delete('/api/deal-feed/:id', (req, res) => {
  try {
...
router.post('/api/deal-feed/save', validate(SaveListingSchema), (req, res) => {
  try {
...
router.delete('/api/deal-feed/save', (req, res) => {
  try {
    const userId    = req.query.userId    ? String(req.query.userId).slice(0, 100) : 'default';
...
router.post('/api/deal-feed/import', validate(ImportListingSchema), (req, res) => {
  try {
...
router.post('/api/deal-feed/ingest/csv', validate(CsvIngestSchema), async (req, res) => {
  try {
...
router.post('/api/deal-feed/:id/score', (req, res) => {
  try {
    const listing = DealFeedService.getListing(req.params.id);
    if (!listing) return errorResponse(res, 404, 'NOT_FOUND', 'Listing not found');
